...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save'])) {
    ...
    if (empty($vorname)) {
    ...
    } elseif (strlen($vorname) > 100) {
        $error = 'Vorname darf maximal 100 Zeichen lang sein.';
    } elseif (empty($name)) {
        $error = 'Nachname ist erforderlich.';
    } elseif (strlen($name) > 100) {
        $error = 'Nachname darf maximal 100 Zeichen lang sein.';
        ...
        if ($id > 0) {
            ...
            $stmt->execute([$vorname, $name, $email, $geburtstag ?: null, $kundeSeit, $geschlecht, $kontaktpermail, $id]);
            ...
            $stmt->execute([$vorname, $name, $email, $geburtstag ?: null, $kundeSeit, $geschlecht, $kontaktpermail]);
                ...
                <form method="post" action="customers.php">
                    ...
                    <div class="form-row form-row-2">
                        <div class="form-group">
                            ...
                            <input type="text" name="vorname" class="form-control"
                                value="<?= h($editCust['vorname'] ?? '') ?>" required>
                        </div>
                        <div class="form-group">
                            <label>Nachname *</label>
                            <input type="text" name="name" class="form-control"
                                value="<?= h($editCust['name'] ?? '') ?>" required>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="email" class="form-control"
                            value="<?= h($editCust['email'] ?? '') ?>" required>
                    </div>
                    ...
                    <div class="form-row form-row-3">
                        <div class="form-group">
                            ...
                            <input type="date" name="geburtstag" class="form-control"
                                value="<?= h($editCust['geburtstag'] ?? '') ?>">
                        </div>
                        <div class="form-group">
                            <label>Kunde seit *</label>
                            <input type="date" name="kunde_seit" class="form-control"
                                value="<?= h($editCust['kunde_seit'] ?? date('Y-m-d')) ?>" required>
                        </div>
                        <div class="form-group">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="td-actions">
                                            ...
                                            <form method="post" action="customers.php"
                                                onsubmit="return confirm('Kunden &quot;<?= h(addslashes($c['vorname'] . ' ' . $c['name'])) ?>&quot; wirklich löschen?');"
